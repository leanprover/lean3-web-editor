...
`elan toolchain install leanprover-community/lean:3.7.1` (replace `3.7.1` with whatever the latest released version is) or `elan toolchain install leanprover-community/lean:nightly` if you prefer to use the nightly. If you then check `elan show`, you should see a new toolchain with the name: `leanprover-community-lean-3.7.1` or `leanprover-community-lean-nightly`. CAVEAT: if you want to bundle mathlib, make sure the version of Lean you install is compatible with the version of mathlib you want to use.
...
3. To make a ZIP bundle containing all of the `.olean` files in one or multiple Lean packages:
    - Edit `combined_lib/leanpkg.toml`:
        - `lean_version` needs to point to the same version of Lean as the Emscripten build of Lean you plan to use. If you've installed the community version of Lean with `elan` as above, then you'll want that line to read `lean_version = "leanprover-community-lean-3.7.1"` (or `lean_version = "leanprover-community-lean-nightly"`).
...
4. To make a ZIP bundle from a single Lean package (containing only the `.olean` files needed for the files in its `src/` directory):
    ...
    - Edit `lean_version` in the package's `leanpkg.toml`. It needs to point to the same version of Lean as the Emscripten build of Lean you plan to use. If you installed the community nightly with `elan` as above, then that line should read `lean_version = "leanprover-community-lean-3.7.1"` (or `lean_version = "leanprover-community-lean-nightly"`).
